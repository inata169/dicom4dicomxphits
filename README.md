...
`dicom4dicomxphits` is the public, data-only companion repository for
...
## Data provenance and publication permission

The source data were generated and exported with Elekta Monaco Release 6.2,
Version 6.2.2.0. They are based on Monaco's non-patient water phantom; the RT
Plans were created by the repository owner, and the corresponding RT Dose
objects were calculated and exported by Monaco.

Elekta confirmed by email that anonymized, non-patient DICOM data generated by
Monaco may be posted to an external community site. Elekta also recommended
stating the Monaco version because the exact version may not be recorded in the
DICOM files themselves. This permission concerns publication of the data and
does not imply Elekta's endorsement of this repository or the clinical validity
of the data, `dicomxphits`, PHITS, or RT-PHITS.
